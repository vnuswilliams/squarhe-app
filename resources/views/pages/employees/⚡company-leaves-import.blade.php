...
new #[Title('Import congés (entreprise)')] class extends Component {
    ...
    public array $columns = [
        'employee_email', 'type', 'start_date', 'end_date', 'notes',
    ];
    ...
    public function updatedFile(): void
    {
        ...
        $this->tempPath = $this->file->store('leave-imports', 'local');
    ...
    public function import(): void
    {
        ...
        Flux::toast(
            variant: 'success',
            text: "{$this->totalRows} congé(s) importé(s) avec succès.",
        );
    }
    ...
    private function rowMessages(): array
    {
        return [
            ...
            'type.in'                 => 'Type de congé invalide (valeurs : ' . implode(', ', LeaveTypeEnum::values()) . ').',
            'start_date.required'     => 'La date de début est obligatoire.',
            'start_date.date'         => 'Date de début invalide.',
            'end_date.required'       => 'La date de fin est obligatoire.',
            'end_date.date'           => 'Date de fin invalide.',
...
<section class="w-full space-y-6">
    ...
    <div class="flex flex-wrap items-center justify-between gap-4">
        <div>
            <flux:heading size="xl">{{ __('Importer des congés') }}</flux:heading>
            <flux:text variant="subtle">Importez les congés de vos employés en masse via un fichier Excel.</flux:text>
        </div>
        <flux:breadcrumbs>
            <flux:breadcrumbs.item href="{{ route('dashboard') }}" icon="home" />
            <flux:breadcrumbs.item href="{{ route('employees') }}">Employés</flux:breadcrumbs.item>
            <flux:breadcrumbs.item>Import congés</flux:breadcrumbs.item>
        </flux:breadcrumbs>
    </div>
    ...
    @if($dispatched)
        ...
        <div class="bg-white/60 dark:bg-zinc-900/60 backdrop-blur-md rounded-2xl border border-zinc-100 dark:border-zinc-800 p-10 flex flex-col items-center gap-4 text-center">
            ...
            <flux:text variant="subtle">
                {{ $totalRows }} congé(s) ont été importés et approuvés.
            </flux:text>
    ...
    @else
        ...
        <div class="bg-white/60 dark:bg-zinc-900/60 backdrop-blur-md p-6 rounded-2xl border border-zinc-100 dark:border-zinc-800 space-y-5">
            ...
            >
                <label for="file-upload-leaves" class="absolute inset-0 cursor-pointer"></label>

                <input
                    x-ref="fileInput"
                    id="file-upload-leaves"
                    type="file"
                    accept=".xlsx,.xls,.csv"
                    wire:model="file"
                    class="sr-only"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
                    <div class="rounded-lg bg-white dark:bg-zinc-900 border border-blue-100 dark:border-zinc-700 p-3 space-y-1.5">
                        <p class="text-xs font-semibold text-zinc-600 dark:text-zinc-400 flex items-center gap-1">
                            <span class="font-mono text-blue-600 dark:text-blue-400">type</span>
                            — valeurs acceptées
                        </p>
                        <div class="flex flex-wrap gap-1">
                            @foreach(\App\Enums\LeaveTypeEnum::cases() as $case)
                    ...
                    <div class="rounded-lg bg-white dark:bg-zinc-900 border border-blue-100 dark:border-zinc-700 p-3 space-y-1.5">
                        ...
                        <div class="flex flex-wrap gap-x-6 gap-y-1 text-[11px] text-zinc-500 dark:text-zinc-400">
                            ...
                            <span><span class="font-mono text-zinc-700 dark:text-zinc-300">start_date / end_date</span> — format <span class="font-mono">YYYY-MM-DD</span></span>
        ...
        <div class="flex items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-3">
                ...
                @if($isValidated && !$hasErrors)
                    <flux:button
                        wire:click="import"
                        wire:confirm="Confirmer l'import de {{ $totalRows }} congé(s) ?"
                        variant="primary"
                        icon="arrow-up-tray"
                    >
