...
new #[Title('Import rémunérations (entreprise)')] class extends Component {
    ...
    public array $columns = [
        'employee_email', 'name', 'amount', 'periodicity', 'impact', 'notes',
    ];
    ...
    public function updatedFile(): void
    {
        ...
        $this->tempPath = $this->file->store('remuneration-imports', 'local');
    ...
    public function import(): void
    {
        ...
        Flux::toast(
            variant: 'success',
            text: "{$this->totalRows} rémunération(s) importée(s) avec succès.",
        );
    }
    ...
    private function rowMessages(): array
    {
        return [
            ...
            'name.required'           => 'Le type de rémunération est obligatoire.',
            'name.in'                 => 'Type de rémunération invalide (valeurs : ' . implode(', ', RemunerationEnum::values()) . ').',
            'amount.required'         => 'Le montant est obligatoire.',
            'amount.numeric'          => 'Le montant doit être numérique.',
            'amount.min'              => 'Le montant doit être ≥ 100.',
            'periodicity.required'    => 'La périodicité est obligatoire.',
            'periodicity.in'          => 'Périodicité invalide (valeurs : ' . implode(', ', PeriodicityEnum::values()) . ').',
            'impact.required'         => 'L\'impact est obligatoire.',
            'impact.in'               => 'Impact invalide (valeurs : ' . implode(', ', ImpactEnum::values()) . ').',
...
<section class="w-full space-y-6">
    ...
    <div class="flex flex-wrap items-center justify-between gap-4">
        <div>
            <flux:heading size="xl">{{ __('Importer des rémunérations') }}</flux:heading>
            <flux:text variant="subtle">Importez les rémunérations de vos employés en masse via un fichier Excel.</flux:text>
        </div>
        <flux:breadcrumbs>
            <flux:breadcrumbs.item href="{{ route('dashboard') }}" icon="home" />
            <flux:breadcrumbs.item href="{{ route('employees') }}">Employés</flux:breadcrumbs.item>
            <flux:breadcrumbs.item>Import rémunérations</flux:breadcrumbs.item>
        </flux:breadcrumbs>
    </div>
    ...
    @if($dispatched)
        ...
        <div class="bg-white/60 dark:bg-zinc-900/60 backdrop-blur-md rounded-2xl border border-zinc-100 dark:border-zinc-800 p-10 flex flex-col items-center gap-4 text-center">
            ...
            <flux:text variant="subtle">
                {{ $totalRows }} rémunération(s) ont été importées et enregistrées.
            </flux:text>
    ...
    @else
        ...
        <div class="bg-white/60 dark:bg-zinc-900/60 backdrop-blur-md p-6 rounded-2xl border border-zinc-100 dark:border-zinc-800 space-y-5">
            ...
            >
                <label for="file-upload-remun" class="absolute inset-0 cursor-pointer"></label>

                <input
                    x-ref="fileInput"
                    id="file-upload-remun"
                    type="file"
                    accept=".xlsx,.xls,.csv"
                    wire:model="file"
                    class="sr-only"
            ...
            <div class="rounded-lg bg-blue-50 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-800/60 p-4 space-y-3">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-1">
                    ...
                    <div class="rounded-lg bg-white dark:bg-zinc-900 border border-blue-100 dark:border-zinc-700 p-3 space-y-1.5">
                        <p class="text-xs font-semibold text-zinc-600 dark:text-zinc-400 flex items-center gap-1">
                            <span class="font-mono text-blue-600 dark:text-blue-400">name</span>
                            — valeurs acceptées
                        </p>
                        <div class="flex flex-wrap gap-1">
                            @foreach(\App\Enums\RemunerationEnum::cases() as $case)
                    ...
                    <div class="rounded-lg bg-white dark:bg-zinc-900 border border-blue-100 dark:border-zinc-700 p-3 space-y-1.5">
                        <p class="text-xs font-semibold text-zinc-600 dark:text-zinc-400 flex items-center gap-1">
                            <span class="font-mono text-blue-600 dark:text-blue-400">periodicity</span>
                            — valeurs acceptées
                        </p>
                        <div class="flex flex-wrap gap-1">
                            @foreach(\App\Enums\PeriodicityEnum::cases() as $case)
                                <span class="inline-flex flex-col items-center rounded px-2 py-1 text-xs
                                    bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700">
                                    <span class="font-mono font-bold text-emerald-600 dark:text-emerald-400">{{ $case->value }}</span>
                                    @if(method_exists($case, 'label'))
                    ...
                    <div class="rounded-lg bg-white dark:bg-zinc-900 border border-blue-100 dark:border-zinc-700 p-3 space-y-1.5">
                        <p class="text-xs font-semibold text-zinc-600 dark:text-zinc-400 flex items-center gap-1">
                            <span class="font-mono text-blue-600 dark:text-blue-400">impact</span>
                            — valeurs acceptées
                        </p>
                        <div class="flex flex-wrap gap-1">
                            @foreach(\App\Enums\ImpactEnum::cases() as $case)
                                <span class="inline-flex flex-col items-center rounded px-2 py-1 text-xs
                                    bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700">
                                    <span class="font-mono font-bold text-emerald-600 dark:text-emerald-400">{{ $case->value }}</span>
                                    @if(method_exists($case, 'label'))
                ...
                <div class="flex flex-wrap gap-x-6 gap-y-1 pt-1 text-[11px] text-zinc-500 dark:text-zinc-400">
                    ...
                    <span><span class="font-mono text-zinc-700 dark:text-zinc-300">amount</span> — nombre ≥ 100</span>
        ...
        <div class="flex items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-3">
                ...
                @if($isValidated && !$hasErrors)
                    <flux:button
                        wire:click="import"
                        wire:confirm="Confirmer l'import de {{ $totalRows }} rémunération(s) ?"
                        variant="primary"
                        icon="arrow-up-tray"
                    >
